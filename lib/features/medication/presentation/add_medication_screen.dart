...
class _AddMedicationScreenState extends ConsumerState<AddMedicationScreen> {
  ...
  void initState() {
    ...
    if (widget.medication != null &&
        widget.medication!.timesPerDay.isNotEmpty) {
      ...
      if (count == 1)
        ...
        _selectedFrequency = 'Three times daily';
      else if (count >= 4)
        _selectedFrequency = 'Four times daily';
    }
  }
  ...
  Future<void> _calculateRefillDate() async {
    ...
    try {
      ...
      final calculatedDate = await refillService.calculateRefillDate(
        tempMedication,
      );
    ...
    if (time != null && !_selectedTimes.contains(time)) {
      setState(() {
        ...
        if (_selectedTimes.length == 1)
          ...
          _selectedFrequency = 'Three times daily';
        else if (_selectedTimes.length >= 4)
          _selectedFrequency = 'Four times daily';
      });
  ...
  Future<void> _saveMedication() async {
    ...
    if (_manualRefillDate && _refillDate == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please select a refill date or disable manual mode'),
        ),
      );
      return;
    }
    ...
    try {
      ...
      // If editing, cancel old notifications
      if (widget.medication != null) {
        ...
        for (int i = 0; i < widget.medication!.timesPerDay.length; i++) {
          ...
          await notificationsService.cancelReminder(
            medicationId.hashCode + i + 100000,
          );
        }
        // Cancel refill reminder
        await notificationsService.cancelRefillReminder(
        ...
        try {
          for (int i = 0; i < _selectedTimes.length; i++) {
            await notificationsService.scheduleRecurringReminder(
              ...
              body:
                  'Time to take ${finalMedication.name} (${finalMedication.dosage})',
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        ...
                        style: ElevatedButton.styleFrom(
                          ...
                          elevation: 0,
                          disabledBackgroundColor: AppTheme.teal500.withValues(
                            alpha: 0.6,
                          ),
                        ),
                        child: _isSaving
